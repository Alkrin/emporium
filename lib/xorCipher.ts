class XORCipher {
  public encode(key: string, data: string): string {
    let midCrypt = this.xor_encrypt(key, data);
    return this.b64_encode(midCrypt);
  }
  ...
  private keyCharAt(key: any, i: any): any {
    return key.charCodeAt(Math.floor(i % key.length));
  }

  private xor_encrypt(key: any, data: any): any {
    return Array.prototype.map.call(data, (c, i) => {
      return c.charCodeAt(0) ^ this.keyCharAt(key, i);
    });
  }

  private xor_decrypt(key: any, data: any): any {
