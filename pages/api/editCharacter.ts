...
export default async function handler(req: IncomingMessage & any, res: ServerResponse & any): Promise<void> {
  try {
    ...
    queries.push({
      query: `UPDATE characters SET user_id=?,name=?,gender=?,portrait_url=?,class_name=?,class_id=?,subclass_id=?,level=?,strength=?,intelligence=?,will=?,dexterity=?,constitution=?,charisma=?,xp=?,hp=?,hit_dice=?,location_id=?,abilities=?,languages=? WHERE id=?`,
      values: [
        b.user_id,
        b.name,
        b.gender,
